...
class Kmeans:
    ...
    def _assign_with_gpu(self, x, nclusters=1):
        ...
        D, I = gpu_index.search(x, nclusters)
        return D, I

    def _assign_without_gpu(self, x, nclusters=1):
        assert self.centroids is not None, "should train before assigning"
        index = faiss.IndexFlatL2(self.d)
        self.centroids = self.centroids.reshape(self.k, self.d)
        index.add(self.centroids)
        D, I = index.search(x, nclusters)
        return D, I
    ...
    def assign(self, x, nclusters=1):
        if self.gpu:
            return self._assign_with_gpu(x, nclusters=nclusters)
        return self._assign_without_gpu(x, nclusters=nclusters)
