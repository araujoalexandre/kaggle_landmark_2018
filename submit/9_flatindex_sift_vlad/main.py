...
class KaggleRetrieval:
  ...
  def __init__(self, debug=False, gpu=False):
    ...
    if debug:
      self.train_files = self.train_files[:50]
  ...
  def _images_iterator(self, images_path):
    ...
    for package in images_path:
      ...
      for filename, loc, desc in features_pack:
        iteration += 1
  ...
  def load_train_by_batch(self):
    ...
    for iteration, filename, desc in \
      ...
      if ndesc_in_batch >= self._BATCH_TRAIN and \
        ...
        print('total number of descriptors reached, kmeans assignement, '
            'conversion to vlad vector and adding to index', flush=True)
        
        xtrain = self._sanitize(np.concatenate(xtrain))
        print(' on xtrain', flush=True)
