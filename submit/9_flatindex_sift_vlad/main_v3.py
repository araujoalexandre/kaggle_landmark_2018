...
class KaggleRetrievalDebug:
  ...
  def _activate_debug_mode(self):
    ...
    self.nimg_train = len(self.train_files * 100)
    self.nimg_test = len(self.test_files * 100)
    self._BATCH_TRAIN = 5000000
...
class KaggleRetrieval(KaggleRetrievalTools, KaggleRetrievalDebug, 
  ...
  def __init__(self, debug=False, gpu=False, valid=True):
    ...
    self.train_files = glob.glob(join(storage_folder, 
      'feature_index_rescale_sift_pkl', '*'))
    self.test_files = glob.glob(join(storage_folder, 
      'feature_test_rescale_sift_pkl', '*'))
  ...
  def _init(self):
    ...
    self.nimg_train = len(self.train_files) * 100
    self.nimg_test = len(glob.glob(join(storage_folder, 'test_rescale', '*')))
  ...
  def _images_iterator(self, images_path):
    iteration = -1
    for package in images_path:
      features_pack = pickle_load(package)
      for filename, _, desc in features_pack:
        if not all([isinstance(filename, str), isinstance(desc, np.ndarray)]):
          print('file {} is corrupted'.format(package))
          filename = ''
          desc = np.zeros((1, 128))
        iteration += 1
        yield iteration, filename, desc[:1000, :]
  ...
  def load_train(self):
    ...
    for iteration, filename, desc in self.train_iterator:
      ...
      vlad = self._make_vlad(desc, predicted)
      self.index.add(self._sanitize(vlad))
