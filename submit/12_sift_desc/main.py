...
class KaggleRetrievalDebug:
  ...
  def _init_debug_mode(self):
    ...
    print('initialization, debug mode activated', flush=True)
    self.train_files = self.train_files[:20000]
    ...
    # init kmeans
    self.kmeans = Kmeans(self.dim, self.ncentroids, niter=10, verbose=True, 
      max_points_per_centroid=None, gpu=self.gpu)
...
class KaggleRetrievalValidation:
  ...
  def _init_validation(self):
    ...
    self.train_files = glob.glob(join(valid_folder, 'feature_train_sift', '*'))
    self.test_files = glob.glob(join(valid_folder, 'feature_test_sift', '*'))
...
class LoadThread(Thread, KaggleRetrievalTools):
  ...
  def run(self):
    global queue
    start = datetime.now()
    for iteration, filename, img in self.train_iterator:
      queue.put((iteration, filename, img))
      if iteration % self._STATUS_CHECK_ITERATIONS == 0 and iteration != 0:
        elapsed = (datetime.now() - start).total_seconds()
        print('load last batch took {:.2f}, queue size {}'.format(
          elapsed, queue.qsize()), flush=True)
        start = datetime.now()
...
class ProcessThread(Thread, KaggleRetrievalTools):
  ...
  def run(self):
    ...
    while True:
      ...
      data, n_desc = [], []
      for x in range(100):
        iteration, filename, img = queue.get()
...
class KaggleRetrieval(KaggleRetrievalTools, KaggleRetrievalDebug, 
  ...
  def __init__(self, debug=False, gpu=False, valid=True):
    ...
    self.train_files = glob.glob(join(kaggle_folder, 'index_rescale', '*'))
    self.test_files = glob.glob(join(kaggle_folder, 'test_rescale', '*'))

    self.dim = 128 # dim of descriptors
  ...
  def _try_load_kmeans(self):
    ...
    if exists(kmeans_checkpoint):
      print('kmeans centroids found, kmeans '
        'training not necessary', flush=True)
  ...
  def _try_load_index(self):
    ...
    if exists(index_checkpoint_full):
      print('full index checkpoint found, loading'
        ' database not necessary', flush=True)
    ...
    # fall back to trained index if it exists
      ...
      print('trained index checkpoint found', flush=True)
  ...
  def compute_kmeans_and_index(self):
    ...
    #   print('kmeans and index already trained', flush=True)
    #   ndesc_in_batch = 0
  ...
  def load_test(self):
    ...
    for iteration, filename, img in self._images_iterator(self.test_files):
      
      # extract desc
      _, desc = sift.detectAndCompute(img, None)
      if not isinstance(desc, np.ndarray):
        print('file {} is corrupted'.format(filename), flush=True)
        desc = np.zeros((1, 128))
