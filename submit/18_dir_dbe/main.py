...
class KaggleRetrieval(KaggleRetrievalTools, KaggleRetrievalDebug, 
  ...
  def __init__(self, debug=False, no_gpu=False, valid=True, qe=0, dbe=0, multires=False):
    ...
    print('query expension : {}'.format(self.qe), flush=True)
    print('database expension : {}'.format(self.dbe), flush=True)
    print('multires : {}'.format(self.multires), flush=True)

    self.images_folder = join(kaggle_folder, 'img')
    self.sample_submission = join(kaggle_folder, 'data', 'sample_submission.csv')
    
    self.train_files = glob.glob(join(storage_folder, 'index', '*'))
    self.test_files = glob.glob(join(storage_folder, 'test', '*'))
    ...
    S = 512
    if self.multires:
      self.sizes = [S - 256, S, S + 256]
    else:
      self.sizes = [S]
    self.img_helper = ImageHelper(S, 2, means)
  ...
  def _try_load_index(self):
    ...
    if exists(index_checkpoint):
      ...
      self.train_filenames = pickle_load(join(self.cur_dir, 'filenames_index.dump'))
  ...
  def load_train(self):
    ...
    for i, path in tqdm(list(enumerate(self.train_files)), file=sys.stdout, leave=False, dynamic_ncols=True):
      filename = splitext(basename(path))[0]
      im = cv2.imread(path)
      for size in self.sizes:
        self.img_helper.S = size
    ...
    xindexes = self._sanitize(xindexes)

    print('saving xindexes', flush=True)
    pickle_dump(xindexes, 'xindexes.pkl')
    ...
    self.index.add(xindexes)
    faiss.write_index(self.index, join(self.cur_dir, 'index_full.dump'))

    if self.dbe:
      print('search knn for each index images', flush=True)
      idx = self.index.search(xindexes, self.dbe + 1)[1]
      weights = np.hstack(([1], (self.dbe - np.arange(0, self.dbe)) / float(self.dbe)))
      weights_sum = weights.sum()
      xindexes_dbe = np.vstack([np.dot(weights, xindexes[idx[i], :]) / weights_sum for i in range(len(xindexes))])
      xindexes_dbe = self._sanitize(xindexes_dbe)

      # save new xindexes_dbe
      print('saving xindexes and idx', flush=True)
      pickle_dump(idx, 'idx.pkl')
      pickle_dump(xindexes_dbe, 'xindexes_dbe.pkl')

      # init new index 
      self.index = faiss.IndexFlatL2(2048)
      self._sanitize(xindexes_dbe)
      self.index.add(xindexes_dbe)
      faiss.write_index(self.index, join(self.cur_dir, 'index_full_dbe.dump'))
    ...
    if qe > 0:
      ...
      for i, pred in tqdm(list(enumerate(predicted)), file=sys.stdout, leave=False, dynamic_ncols=True):
        for index in pred[:qe]:
          ...
          norm += 1
        query_database[i] /= norm
      pickle_dump(query_database, 'xqueries_qe.pkl')
...
def main():
  ...
  parser.add_argument("--qe",  type=int, default=0)
  parser.add_argument("--dbe",  type=int, default=0)

  args = parser.parse_args()
  kaggle = KaggleRetrieval(**vars(args))
  kaggle.load_train()
